...
export function getPrData(pr: PullRequest): PrData | undefined {
    ...
    const date = new Date(pr.merged_at ?? pr.updated_at).toLocaleDateString('en-US', dateFormatOptions)

    const match = content.match(/## @fire-ui\/react\@(?<version>\d.+)/)
    ...
    const body = [
        ...
        `description: Explore the changelog for FireUI version ${ version }. Learn about the latest features, bug fixed and improvements.`,
...
export async function getLatestPr(): Promise<PullRequest> {
    const { data } = await octokit.pulls.list({
        ...
        repo: 'fire-ui-react',
        base: 'master',
        head: 'fire-ui-react:changeset-release/master',
        per_page: 1
    })
...
export async function writeReadme() {
    ...
    const sortedData = data.map((pr) => `### ${ pr.date }: [v${ pr.version }(/.changelog/v${ pr.version }.mdx)]`)

...
export const reactPackage = '@fire-ui/react'
export const docsPackage = '@fire-ui/docs'

export function getPackageName(name: string) {
    return startCase(name.replace('@fire-ui/', ''))
}
...
export function getReleaseSummary(changesets: NewChangeset[], release: ComprehensiveRelease) {
    ...
    const subPackageName = `**${ getPackageName(release.name) }** \`v${ release.newVersion }\``

    const rootPackageName = `\`${ reactPackage }@${ release.newVersion }\``
    const displayName = release.name === reactPackage ? rootPackageName : subPackageName
...
export async function getChangesetEntries(options: Options) {
    const { cwd, sinceRef, exclude = [docsPackage] } = options

    const releasePlan = await getReleasePlan(cwd, sinceRef)

    const releases = releasePlan.releases
        .filter((release) => release.name === reactPackage || releases.changesets.length > 0)
        .filter((release) => !exclude.includes(release.name))
        .sort((a, b) => {
            if(a.name === reactPackage) return -1
            if(b.name === reactPackage) return 1
    ...
    return {
        releases,
        summary: releases.map((release) => getReleaseSummary(releasePlan.changesets, release))
    }
}
import { theme } from '@fire-ui/theme'
...
type ComponentTypeProperties = {
    [component: string]: ComponentTpyeInfo
}
...
async function extractPropertiesOfTypeName(searchTerm: string | RegExp, sourceFile: ts.SourceFile, typeChecker: ts.TypeChecker, { shouldIgnoreProperty = () => false }: TypeSearchOptions = { }) {
    ...
    for(const typeStatement of typeStatements) {
        ...
        for(const property of type.getProperties()) {
            ...
            const defaultValue = docTags.find((tag) => tag.name === 'default')?.text?.map((doc) => doc.text)?.join('\n') || undefined
            ...
            properties[propertyName] = {
                ...
                required,
                description: property.getDocumentationComment(typeChecker).map((comment) => comment.text).join('\n') || undefined
            }
        }
...
function createTypeSearch(tsConfigPath: string, typeSearchOptions: TypeSearchOptions = { }) {
    ...
    return (searchTerm: Parameters<typeof extractPropertiesOfTypeName>[0]) => {
        ...
        for(const sourceFile of sourceFiles) {
            const typeInfo = extractPropertiesOfTypeName(searchTerm, sourceFile, program.getTypeChecker(), { shouldIgnoreProperty })
...
function shouldIgnoreProperty(property: ts.Symbol) {
    ...
    const isExternal = /(node_modules|styled-system|system)/.test(sourceFileName ?? '')
...
const main = async () => {
    ...
    const typeExports = extractTypeExports(fileContent)?.map(searchType)

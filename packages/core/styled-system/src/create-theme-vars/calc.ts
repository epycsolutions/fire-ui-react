...
const toExpresssion = (operator: Operator, ...operands: Array<Operand>) =>
    operands.map(resolveReference).join(` ${ operator } `).replace(/calc/g, '')

const add = (...operands: Array<Operand>) => `calc(${ toExpresssion('+', ...operands) })`

const subtract = (...operands: Array<Operand>) => `calc(${ toExpresssion('-', ...operands) })`

const multiply = (...operands: Array<Operand>) => `calc(${ toExpresssion('*', ...operands) })`

const divide = (...operands: Array<Operand>) => `calc(${ toExpresssion('/', ...operands) })`

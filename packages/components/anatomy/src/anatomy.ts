 ...
 * consistent APIs for `className`, css selector and `theming`
 */
export function anatomy<T extends string = string>(
    ...
    function toPart(part: string) {
        const element = ['container', 'root'].includes(part ?? '')
            ? [name]
            : [name, part]
        const attribute = element.filter(Boolean).join('__')
        const className = `fire-${attribute}`

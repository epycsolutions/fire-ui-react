 ...
 * @param length the current length or cound of items
 ...
 */
export function getNextItemFromSearch<T>(items: T[], searchString: string, itemToString: (item: T) => string, currentItem: T): T | undefined {
    if(searchString == null) return currentItem

    // If current item doesn't exists, find the item that matches the search string
    if(!currentItem) {
        const foundItem = items.find((item) => itemToString(item).toLowerCase().startsWith(searchString.toLowerCase()))
        return foundItem
    }

    // Filter items for ones that match the search string (case intensitive)
    ...
    // a decent fallback to current item
    return currentItem
}